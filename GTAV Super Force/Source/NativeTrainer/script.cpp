...
//start: Matthew Guidry 2017
//super punch methods
//closest vehicle finder
...
bool isVehicleDrivable(Vehicle veh) {
	if (VEHICLE::IS_VEHICLE_DRIVEABLE(veh, false) && !VEHICLE::IS_THIS_MODEL_A_TRAIN(getModelHash(veh)))
	{
		return true;
	}
	return false;
...
double force = 1000.0;
double pedForce =  10.0;

void superPunchPed(Ped playerPed, Ped trueped)
{
  //get camera rotation
	...
	direction.x = pedForce * direction.x;
	direction.y = pedForce * direction.y;
	direction.z = pedForce * direction.z;

	PED::SET_PED_TO_RAGDOLL(trueped, 0, 0, 0, TRUE, TRUE, FALSE);
	//apply the force to the entity
	//rotation is a normalized vector to rotate the object
	ENTITY::APPLY_FORCE_TO_ENTITY(trueped, 1, direction.x, direction.y, direction.z, Rot.x, Rot.y, Rot.z, false, false, true, true, false, true);
	ENTITY::CLEAR_ENTITY_LAST_DAMAGE_ENTITY(trueped);
}

void superPunchVeh(Ped playerPed, Vehicle vehobj)
{
	//get camera rotation
	Vector3 Rot = CAM::GET_GAMEPLAY_CAM_ROT(2);
	//normalize to directional vector
	...
	//apply the force to the entity
	//rotation is a normalized vector to rotate the object
	ENTITY::APPLY_FORCE_TO_ENTITY(vehobj, 1, direction.x, direction.y, direction.z, Rot.x, Rot.y, 30.0, false, false, true, true, false, true);
	ENTITY::CLEAR_ENTITY_LAST_DAMAGE_ENTITY(vehobj);
}

Vehicle getDamagedVehicle(Ped ped) {
	Vehicle nearveh = NULL;
	const int ARR_SIZE = 1024;
	Vehicle vehs[ARR_SIZE];
	int countv = worldGetAllVehicles(vehs, ARR_SIZE);
	Ped player = PLAYER::PLAYER_PED_ID();
	if (vehs != NULL)
	{
		for (int i = 0; i < countv; i++)
		{
			if (doesEntityExistsAndIsNotNull(vehs[i]) && ENTITY::HAS_ENTITY_BEEN_DAMAGED_BY_ENTITY(vehs[i], player, -1))
			{
				nearveh = vehs[i];
				superPunchVeh(player, nearveh);
			}
		}
	}
	return nearveh;
...
Vehicle getDamagedPed(Ped ped) {
	Ped nearped = NULL;
	const int ARR_SIZE = 1024;
	Ped peds[ARR_SIZE];
	int count = PED::GET_PED_NEARBY_PEDS(ped, peds, 0);
	Ped player = PLAYER::PLAYER_PED_ID();
	if (peds != NULL)
	{
		for (int i = 0; i < count; i++)
		{
			if (doesEntityExistsAndIsNotNull(peds[i]) && (peds[i] != player) && ENTITY::HAS_ENTITY_BEEN_DAMAGED_BY_ENTITY(peds[i], player, -1))
			{
				nearped = peds[i];
				superPunchPed(player, nearped);
			}
		}
	}
	return nearped;
...
{
	Ped player = PLAYER::PLAYER_PED_ID();
	Ped nearPed = getDamagedPed(player);
	if (nearPed != NULL)
	{
		superPunchPed(player, nearPed);
	}

	Vehicle nearVeh = getDamagedVehicle(player);
	if (nearVeh != NULL)
	{
		superPunchVeh(player, nearVeh);
	}
...
void draw_menu_line(std::string caption, float lineWidth, float lineHeight, float lineTop, float lineLeft, float textLeft, bool active, bool title, bool rescaleText = true)
{
	...
	//scale menu with screen resolution
...
//boolean method to check if trainer activate button pressed(F4)
...
// features
bool featurePlayerSuperPunch			=	false;
...
void process_player_menu()
{
	...
	static struct {
		...
		{"TP INTO NEAREST VEHICLE", NULL, NULL},
		...
		{"SUPER PUNCH", &featurePlayerSuperPunch,NULL},
	...
	while (true)
	{
		...
		{
			// draw menu
			draw_menu_line(caption, lineWidth, 16.0, 18.0, 0.0, 5.0, false, true);
			for (int i = 0; i < lineCount; i++)
		...
		if (bSelect)
		{
			...
			switch (activeLineIndexPlayer)
			{
				...
				case 7:
					{
						Vehicle ranveh = getClosestVehicleFromPedPos(playerPed, 9999.9004, 9999.9004, FALSE);
						if (!VEHICLE::IS_VEHICLE_SEAT_FREE(ranveh, -1))
						{
							Ped *baneOfYourExistance = (Ped *) VEHICLE::GET_PED_IN_VEHICLE_SEAT(ranveh, -1);
							PED::DELETE_PED(baneOfYourExistance);
						}
						ENTITY::SET_ENTITY_HEADING(ranveh, bPlayerExists);
						PED::SET_PED_INTO_VEHICLE(playerPed, ranveh, -1);
						set_status_text("teleported");
					}
